...
const fixtureUnit = {
  id: 'fixtureUnit', class: 'ground', footprint: [64, 64, 16],
  camera: { azimuth: 45, elevation: 30 }, layerSpacing: 2,
  parts: [
    { id: 'body', kind: 'directional', facings: 8, atlas: 'fixtureUnit.body.png', cell: [64, 64], pivot: [32, 44], zeroFacing: '+x' },
    { id: 'turret', kind: 'stack', angles: 64, atlas: 'fixtureUnit.turret.png', cell: [64, 64], pivot: [32, 44], mount: [0, 0, 9] },
  ],
  shadow: { kind: 'ellipse', rx: 33, ry: 14, alt: 0 }, stats: { speed: 90, turnRate: 3, turretRate: 4 },
};

const v = validatePack(fixtureUnit);
ok('valid Abrams pack', v.ok, JSON.stringify(v.errors));
ok('partById turret', partById(fixtureUnit, 'turret')?.kind === 'stack');

// broken packs must be caught
ok('missing camera caught', !validatePack({ ...fixtureUnit, camera: null }).ok);
ok('bad class caught', !validatePack({ ...fixtureUnit, class: 'boat' }).ok);
ok('stack without angles caught', !validatePack({ ...fixtureUnit, parts: [{ id: 't', kind: 'stack', atlas: 'a', cell: [1, 1], pivot: [0, 0] }] }).ok);
ok('empty parts caught', !validatePack({ ...fixtureUnit, parts: [] }).ok);
